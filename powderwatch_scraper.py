...
def parse_snow_report(html, resort_name):
    ...
    m_json = re.search(r'<script id="__NEXT_DATA__" type="application/json">(.*?)</script>', html, re.DOTALL)
    if m_json:
        try:
            json_str = m_json.group(1)
            m_base = re.search(r'"baseDepth"\s*:\s*(\d+)|"base"\s*:\s*(\d+)', json_str, re.IGNORECASE)
            if m_base: d["base_depth"] = int(m_base.group(1) or m_base.group(2))
            m_lifts_open = re.search(r'"liftsOpen"\s*:\s*(\d+)', json_str, re.IGNORECASE)
            if m_lifts_open: d["lifts_open"] = int(m_lifts_open.group(1))
            m_lifts_total = re.search(r'"liftsTotal"\s*:\s*(\d+)', json_str, re.IGNORECASE)
            if m_lifts_total: d["lifts_total"] = int(m_lifts_total.group(1))
            m_7d = re.search(r'"snowfall7(?:Days|d)?"\s*:\s*(\d+(?:\.\d+)?)', json_str, re.IGNORECASE)
            if m_7d:
                val = float(m_7d.group(1))
                if val > 0: d["snowfall_7d"] = val
            m_24h = re.search(r'"snowfall24(?:Hours|h)?"\s*:\s*(\d+(?:\.\d+)?)', json_str, re.IGNORECASE)
            if m_24h:
                val = float(m_24h.group(1))
                if val > 0: d["snowfall_24h"] = val
            m_status = re.search(r'"status"\s*:\s*"([^"]+)"', json_str, re.IGNORECASE)
            if m_status: d["status"] = m_status.group(1).title()
        except Exception:
            pass
    ...
    if d["status"] == "Unknown":
        m = re.search(r':\s*(Open|Closed).*?Last Updated:\s*([A-Za-z]+\s+\d+)', text, re.IGNORECASE)
        if m:
            d["status"] = m.group(1).title()
            d["last_updated"] = m.group(2)

    m_visual_lifts = re.search(r'Lifts(?:\s+Open)?[^\d]{0,30}?(\d+)\s*(?:/|of)\s*(\d+)', text, re.IGNORECASE)
    if m_visual_lifts:
        d["lifts_open"] = int(m_visual_lifts.group(1))
        d["lifts_total"] = int(m_visual_lifts.group(2))
    elif d["lifts_total"] == 0 or d["lifts_open"] == 0:
        m = re.search(r'with\s+(?:(\d+)\s+)?of\s+(\d+)\s+lifts', text, re.IGNORECASE)
        if m:
            if m.group(1): d["lifts_open"] = int(m.group(1))
            d["lifts_total"] = int(m.group(2))

    m_visual_runs = re.search(r'Runs\s+Open[^\d]{0,20}?(\d+)\s*/\s*(\d+)', text, re.IGNORECASE)
    if m_visual_runs:
        d["runs_open"] = int(m_visual_runs.group(1))
        d["runs_total"] = int(m_visual_runs.group(2))

    if d["base_depth"] == 0:
        m = re.search(r'Base[^\d]{0,20}?(\d+)\s*"', text, re.IGNORECASE)
        if m: d["base_depth"] = int(m.group(1))

    if d["mid_depth"] == 0:
        m = re.search(r'Mid\s*Mountain[^\d]{0,20}?(\d+)\s*"', text, re.IGNORECASE)
        if m: d["mid_depth"] = int(m.group(1))
    if d["summit_depth"] == 0:
        m = re.search(r'Summit[^\d]{0,20}?(\d+)\s*"', text, re.IGNORECASE)
        if m: d["summit_depth"] = int(m.group(1))

    if not d["surface_condition"]:
        m = re.search(r'Surface[^\d]{0,20}?(Powder|Packed Powder|Machine Groomed|Loose Granular|Hard Packed|Spring Snow|Corn Snow)', text, re.IGNORECASE)
        if m:
            d["surface_condition"] = m.group(1).title()

    idx = text.lower().find("recent snowfall")
    if idx != -1:
        sub_text = text[idx:idx+400]
        day_words = r'\b(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun|24h|48h|72h|7d)\b'
        headers_pattern = rf'({day_words}(?:[^a-zA-Z0-9]+{day_words})*)'
        amounts_pattern = r'((?:\d+(?:\.\d+)?\s*"(?:[^a-zA-Z0-9]+)?)+)'
        table_match = re.search(headers_pattern + r'[^\d]*?' + amounts_pattern, sub_text, re.IGNORECASE)
        if table_match:
            headers = re.findall(day_words, table_match.group(1), re.IGNORECASE)
            amounts = re.findall(r'(\d+(?:\.\d+)?)\s*"', table_match.group(2))
            if headers and amounts:
                days_total = 0.0
                has_days = False
                visual_24h = visual_7d = None
                for h, a in zip(headers, amounts):
                    h_lower = h.lower()
                    if h_lower in ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun']:
                        days_total += float(a); has_days = True
                    elif h_lower == '24h': visual_24h = float(a)
                    elif h_lower == '7d': visual_7d = float(a)
                if d["snowfall_24h"] == 0 and visual_24h is not None: d["snowfall_24h"] = visual_24h
                if d["snowfall_7d"] == 0:
                    if visual_7d is not None: d["snowfall_7d"] = visual_7d
                    elif has_days: d["snowfall_7d"] = round(days_total, 1)

    if d["lifts_open"] == 0 and d["lifts_total"] > 0: d["status"] = "Closed"
    return d
...
def scrape_all():
    ...
    for i, resort in enumerate(RESORTS):
        ...
        if i < total - 1: time.sleep(REQUEST_DELAY)
    return results
